...
public class LevelManager : MonoBehaviour
{
    ...
    private const int MAX_SEGMENTS_ONSCREEN = 15;
    private Transform cameraCointer;
    ...
    {
        Instance = this;
        //cameraCointer = Camera.main.transform;
        currentSpawnZ = 0;
        currentLevel = 0;
    }
    ...
    private void Update()
    {
        if (currentSpawnZ - cameraCointer.position.z < DISTANCE_BEFORE_SPAWN)       
            GenerateSegment();

        if(amountOfActiveSegments >= MAX_SEGMENTS_ONSCREEN)
        {
